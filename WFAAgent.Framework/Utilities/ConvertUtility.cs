...
namespace WFAAgent.Framework.Utilities
{
    public class ConvertUtility
    {
        ...
        public static long ByteArrayToLong(byte[] value, int startIndex = 0)
        {
            if (value == null)
            {
                throw new ArgumentNullException(nameof(value));
        ...
        public static int[] StringArrayToIntArray(string[] value)
        {
            int[] retValue = new int[value.Length];
            for (int i = 0; i < value.Length; i++)
            {
                retValue[i] = int.Parse(value[i]);
            }
            return retValue;
        }
    }
